import React, { Component } from "react";

class JuntaDirectiva extends Component {
	render() {
		return (<>
			<section>
                <h2>Junta directiva</h2>

				<p>
					Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nam tincidunt enim volutpat elit ullamcorper commodo. Nam tristique dictum justo quis elementum. Praesent auctor scelerisque lacinia. Nulla nisi justo, commodo ut pulvinar quis, imperdiet nec dolor. Curabitur id felis nec mauris tempus tempus tincidunt eu leo. Praesent mattis ac lacus in pulvinar. Phasellus id mauris blandit, pellentesque mi eget, tincidunt ligula.
				</p>

				<div className="juntes tecnica">
					<div className="junta-column">
						<div className="junta-team">
							<h4>Direcció tècnica</h4>
							<p><span>Presidenta</span>: Clara Montoya García</p>
							<p><span>Vice-president</span>: Benet Antius Lozano</p>
							<p><span>Secretàries</span>: Cristina Oliveira Coll i Aina Oliu Cordomí</p>
						</div>
					</div>
					<div className="junta-column">
						<div className="junta-team">
							<h4>Tresoreria</h4>
							<p><span>Tresorer</span>: Pere Llopart</p>
							<p>Hèctor Carmona López</p>
							<p>Guillem Canals</p>
						</div>

						<div className="junta-team">
							<h4>Promoció i difusió</h4>
							<p><span>Cap de promoció</span>: Núria Fonts</p>
							<p>Martina Soler Collell</p>
							<p>Arianda Sánchez Rodríguez</p>
							<p>Jordi Casañas Dalmau</p>
							<p>Ermengol Passola Lizandra</p>
							<p>Jordina Rué Alcové</p>
						</div>

						<div className="junta-team">
							<h4>Logística i contractació</h4>
							<p><span>Cap de logística</span>: Carla Rubio Masferrer</p>
							<p>Jaume Oriol Lladó</p>
							<p>Arnau Mallén Boronat</p>
							<p>Eduard Càmara Frias</p>
							<p>Martí Gumà Marín</p>
							<p>Clara Llonch Pulido</p>
						</div>

						<div className="junta-team">
							<h4>Equip d'acollida</h4>
							<p><span>Cap d'acollida</span>: Maria Babià Soler</p>
							<p>Pere Llopart</p>
							<p>Aida Sanfeliu</p>
							<p>Jordina Rué Alcové</p>
							<p>Núria Fonts</p>
							<p>Aina Oliu Cordomí</p>
							<p>Anna Santamaria Balaguer</p>
							<p>Queralt Casademont</p>
							<p>Gina Giró Aguilar</p>
							<p>Berta Ramírez Jou</p>
						</div>

						<div className="junta-team">
							<h4>Gralles i tabals</h4>
							<p><span>Cap de músics</span>: Genís Soler Rafart</p>
							<p>Oriol Pérez Fernández</p>
							<p>Blai Bolea Galvez</p>
						</div>
					</div>
					<div className="junta-column">
						<div className="junta-team">
							<h4>Sanitaris</h4>
							<p><span>Cap de sanitaris</span>: Laia Soto Gustamante</p>
							<p>Laura Llopis Colom</p>
							<p>Maria Llorach Pulido</p>
							<p>Blai Rosell Ollé</p>
							<p>Ares Font Urpí</p>
							<p>David Pato</p>
							<p>Laura Baus</p>
						</div>

						<div className="junta-team">
							<h4>Material</h4>
							<p><span>Cap de material</span>: Sabina García Sàbat</p>
							<p>Queralt Casademont</p>
							<p>Laura Ribó Roca</p>
							<p>Marta Arnau Piculla</p>
						</div>

						<div className="junta-team">
							<h4>Activitats</h4>
							<p><span>Cap d'activitats</span>: Pere Llopart</p>
							<p>Ermengol Passola Lizandra</p>
							<p>Andreu Huguet Segarra</p>
							<p>Edgar Tena Marqués</p>
							<p>Jordina Rué Alcové</p>
							<p>Anna Santamaria Balaguer</p>
						</div>

						<div className="junta-team">
							<h4>Comissió feminista</h4>
							<p><span>Cap</span>: Clàudia Massachs</p>
							<p>Edgar Tena Marqués</p>
							<p>Bet Romera Mirabent</p>
							<p>Marina de la Torre Lozano</p>
						</div>
					</div>
				</div>
			</section>
		</>);
	}
}

export default JuntaDirectiva;
